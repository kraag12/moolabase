...
type ListingPageClientProps = {
  ...
  initialBoostPaymentId?: string
  initialBoostStatus?: string
}

export default function ListingPageClient({
  initialId = '',
  initialType = '',
  initialBoostPaymentId = '',
  initialBoostStatus = '',
}: ListingPageClientProps) {
  const id = initialId.trim()
  const type = initialType.trim() as ListingType | ''
  const boostPaymentId = initialBoostPaymentId.trim()
  const boostStatus = initialBoostStatus.trim().toLowerCase()
