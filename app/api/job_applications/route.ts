...
async function getJobWithOwner(supabase: any, jobId: string) {
  const ownerColumn = await resolveColumn(supabase, 'jobs', 'poster_id', 'user_id')
  ...
  return { job: data, ownerId: data?.[ownerColumn] as string | null }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { ownerId } = await getJobWithOwner(supabase, jobId)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { job_id, motivation, message } = body
    ...
    const { ownerId } = await getJobWithOwner(supabase, job_id)
    if (ownerId && ownerId === userId) {
...
export async function PATCH(request: NextRequest, { params }: { params?: { id?: string } }) {
  try {
    ...
    if (current.listing_type !== 'job') {
      return NextResponse.json({ error: 'Invalid application type' }, { status: 400 })
    }

    const { ownerId } = await getJobWithOwner(supabase, current.listing_id)
    if (!ownerId || ownerId !== userId) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
    }
