...
export async function GET(
  ...
  try {
    ...
    if (!isSupportedListingId(id)) {
      console.warn('GET /api/services/[id] called with invalid id:', id)
    ...
    if (record && isListingExpired(record?.created_at, record?.duration)) {
      const cleanupClient = createCleanupClient()
      if (cleanupClient) {
        const { error: deleteError } = await cleanupClient.from('services').delete().eq('id', id)
        if (deleteError) {
          console.warn('Failed to delete expired service:', deleteError.message || deleteError)
        }
      }
      record = null
    }
